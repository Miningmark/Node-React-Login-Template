                        ...
                        <div className="card-body">
                            <form onSubmit={handleUpdatePassword}>
                                ...
                                <div className="form-floating mb-3">
                                    <input
                                        type="password"
                                        className={`form-control ${touched.password ? (isLengthValid && hasLowercase && hasUppercase && hasNumber && hasSpecialChar ? "is-valid" : "is-invalid") : ""}`}
                                        id="floatingNewPassword"
                                        placeholder="Neues Passwort"
                                        value={newPassword}
                                        onChange={(e) => setNewPassword(e.target.value)}
                                        onBlur={(e) => setTouched({ ...touched, password: e.target.value === "" ? false : true })}
                                        required
                                        ref={passwordRef}
                                    />
                                ...
                                <ul className="mb-3 list-unstyled small">
                                    <li className={touched.password ? (isLengthValid ? "text-success" : "text-danger") : ""}>{isLengthValid ? "✅" : "❌"} Mindestens 8 Zeichen, max. 24 Zeichen</li>
                                    <li className={touched.password ? (hasLowercase ? "text-success" : "text-danger") : ""}>{hasLowercase ? "✅" : "❌"} Mindestens ein Kleinbuchstabe</li>
                                    <li className={touched.password ? (hasUppercase ? "text-success" : "text-danger") : ""}>{hasUppercase ? "✅" : "❌"} Mindestens ein Großbuchstabe</li>
                                    <li className={touched.password ? (hasNumber ? "text-success" : "text-danger") : ""}>{hasNumber ? "✅" : "❌"} Mindestens eine Zahl</li>
                                    <li className={touched.password ? (hasSpecialChar ? "text-success" : "text-danger") : ""}>{hasSpecialChar ? "✅" : "❌"} Mindestens ein Sonderzeichen</li>
                                </ul>
                                <div className="form-floating mb-3">
                                    <input
                                        type="password"
                                        className={`form-control ${touched.repeat && !passwordsMatch ? "is-invalid" : passwordsMatch && confirmPassword !== "" ? "is-valid" : ""}`}
                                        id="floatingConfirmPassword"
                                        placeholder="Passwort wiederholen"
                                        value={confirmPassword}
                                        onChange={(e) => setConfirmPassword(e.target.value)}
                                        onBlur={(e) => setTouched({ ...touched, repeat: e.target.value === "" ? false : true })}
                                        required
                                        ref={repeatRef}
                                    />
                                    <label htmlFor="floatingConfirmPassword">Passwort wiederholen</label>
                                    {touched.repeat && !passwordsMatch && <div className="invalid-feedback">Passwörter stimmen nicht überein.</div>}
                                </div>
                                <button className="btn btn-primary w-100" type="submit" disabled={loadingPassword}>
                                    {loadingPassword ? <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> : "Passwort aktualisieren"}
                                </button>
                            </form>
                        </div>
                    </div>
                            ...
                            <form onSubmit={handleUpdateEmail}>
                                <div className="form-floating mb-3">
                                    <input
                                        ...
                                        onBlur={(e) => setTouched({ ...touched, email: e.target.value === "" ? false : true })}
                                        required
                                        ref={emailRef}
                                    />
                                ...
                                <button className="btn btn-primary w-100" type="submit" disabled={loadingEmail}>
                                    {loadingEmail ? <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> : "E-Mail aktualisieren"}
                                </button>
                            </form>
                        </div>
                    </div>
                            ...
                            <div className="card-body">
                                <form onSubmit={handleUpdateUsername}>
                                    <div className="form-floating mb-3">
                                        <input
                                            type="text"
                                            className={`form-control ${touched.username ? (isUsernameValid ? "is-valid" : "is-invalid") : ""}`}
                                            id="floatingNewUsername"
                                            placeholder="Benutzername"
                                            value={newUsername}
                                            onChange={(e) => setNewUsername(e.target.value)}
                                            onBlur={(e) => setTouched({ ...touched, username: e.target.value === "" ? false : true })}
                                            required
                                            ref={usernameRef}
                                        />
                                        <label htmlFor="floatingNewUsername">Benutzername</label>
                                    </div>
                                    <button className="btn btn-primary w-100" type="submit" disabled={loadingUsername}>
                                        {loadingUsername ? <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> : "Benutzername aktualisieren"}
                                    </button>
                                </form>
                            </div>
                        </div>
                                        ...
                                        <tbody>
                                            {logins.map((entry, idx) => (
                                                <tr key={idx}>
                                                    ...
                                                    <td>{entry.regionName === "IP Lookup nicht erfolgreich" ? entry.regionName : `${entry.regionName} / ${entry.country}`}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
