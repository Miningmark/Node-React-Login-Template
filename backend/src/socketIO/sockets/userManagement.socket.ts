...
export default (socket: Socket<ClientToServerEvents, ServerToClientEvents, any>) => {
    socket.on("subscribe:userManagement:users:watchList", () => {
        if (socket.routeGroups.some((routeGroup) => [USER_MANAGEMENT_READ.groupName, USER_MANAGEMENT_WRITE.groupName, USER_MANAGEMENT_CREATE.groupName].includes(routeGroup))) {
