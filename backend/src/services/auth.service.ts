...
export class AuthService {
    ...
    async register(username: string, email: string, password: string): Promise<ControllerResponse> {
        ...
        if (databaseUser !== null)
            throw new ValidationError("Benutzername oder Email bereits vergeben");
        ...
        const token = await this.tokenService.generateHexUserToken(
            ...
            ENV.ACCOUNT_ACTIVATION_USER_EXPIRY
        );
    ...
    async login(
        ...
        const jsonResponse: Record<string, any> = { message: "Benutzer erfolgreich angemeldet" };
        ...
        if (databaseUser.isActive === false)
            throw new ForbiddenError(
                "Benutzer ist noch nicht aktiviert oder vorübergehen deaktiviert"
            );
    ...
    async logout(userId: number, res: Response): Promise<ControllerResponse> {
        const jsonResponse: Record<string, any> = { message: "Benutzer erfolgreich abgemeldet" };
    ...
    async accountActivation(token: string): Promise<ControllerResponse> {
        ...
        const databaseUserToken = await UserToken.findOne({
            where: { type: UserTokenType.USER_REGISTRATION_TOKEN, token: token },
            include: { model: User }
        });
        if (databaseUserToken === null || databaseUserToken.user === undefined)
            throw new ValidationError("Token nicht vorhanden oder ungültig");

        if (
            databaseUserToken.expiresAt !== null &&
            new Date(Date.now()) > databaseUserToken.expiresAt
        ) {
        ...
        this.socketService.emitToRoom(
            "listen:userManagement:users:watchList",
            "userManagement:users:update",
            this.userService.generateJSONResponse(
                databaseUserToken.user.id,
                undefined,
                undefined,
                true
            )
    ...
    async refreshAccessToken(token: string, res: Response): Promise<ControllerResponse> {
        const jsonResponse: Record<string, any> = { message: "Token erfolgreich erneuert" };

        const refreshUserToken = await UserToken.findOne({
            where: { type: UserTokenType.REFRESH_TOKEN, token: token },
            include: { model: User }
        });
        ...
        const accessUserToken = await UserToken.findOne({
            where: { type: UserTokenType.ACCESS_TOKEN, userId: refreshUserToken.user.id }
        });
        ...
        try {
            ...
            if (
                ...
                decoded.userId !== refreshUserToken.user.id
            ) {
    ...
    async requestPasswordReset(usernameOrEmail: string): Promise<ControllerResponse> {
        const jsonResponse: Record<string, any> = {
            message: "Email zum Passwort ändern erfolgreich versandt"
        };
        ...
        if (databaseUser === null)
            throw new ValidationError(
                "Es existiert kein Benutzer mit diesem Benutzername oder Email"
            );
        if (databaseUser.isDisabled)
            throw new ForbiddenError(
                "Benutzer gesperrt, zurücksetzten des Passworts nicht möglich"
            );

        const databaseUserToken = await UserToken.findOne({
            where: { userId: databaseUser.id, type: UserTokenType.PASSWORD_RESET_TOKEN }
        });
    ...
    async handlePasswordRecovery(token: string, password: string): Promise<ControllerResponse> {
        const jsonResponse: Record<string, any> = { message: "Passwort erfolgreich gespeichert" };
        ...
        if (databaseUserToken === null || databaseUserToken.user === undefined)
            throw new ValidationError("Token nicht vorhanden oder abgelaufen");
        ...
        if (
            ...
            if (databaseUserToken.type === UserTokenType.ADMIN_REGISTRATION_TOKEN) {
                ...
                throw new UnauthorizedError(
                    "Zeit zum abschließen der Registrierung leider abgelaufen bitte selbst registrieren oder einen Admin darum bitten"
                );
            }
            ...
            throw new ValidationError(
                "Zeit zum zurücksetzten leider schon abgelaufen, es wurde Ihnen eine neue Email gesendet"
            );
        }
        ...
        if (isPasswordMatching === true)
            throw new ValidationError(
                "Ihr neues Passwort kann nicht mit ihrem alten Passwort gleich sein"
            );
