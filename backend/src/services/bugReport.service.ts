...
export class BugReportService {
    ...
    async getBugReports(limit?: number, offset?: number): Promise<ControllerResponse> {
        const jsonResponse: Record<string, any> = {
            message: "BugReports erfolgreich zurückgegeben"
        };
    ...
    async getActiveBugReports(limit?: number, offset?: number): Promise<ControllerResponse> {
        const jsonResponse: Record<string, any> = {
            message: "Alle aktiven BugReports erfolgreich zurückgegeben"
        };
        const databaseBugReports = await BugReport.findAll({
            where: {
                status: {
                    [Op.or]: [BugReportStatusType.CONFIRMED, BugReportStatusType.IN_PROGRESS]
                }
            },
            ...(limit !== undefined && offset !== undefined
                ? { limit: limit, offset: offset }
    ...
    async getBugReportFile(
        ...
        const jsonResponse: Record<string, any> = {
            message: "Datei für BugReport erfolgreich zurückgegeben"
        };
        let stream, contentType;

        const databaseBugReport = await BugReport.findOne({ where: { id: id } });

        if (databaseBugReport === null)
            throw new ValidationError("Keinen BugReport mit dieser ID gefunden");
        if (
            databaseBugReport.userId !== userId &&
            routeGroups.includes(BUG_REPORT_READ.groupName) === false
        )
            throw new ForbiddenError("Du kannst nur Dateien von deinen eigenen BugReports ansehen");
        if (
            databaseBugReport.fileNames.length === 0 ||
            databaseBugReport.fileNames.length < fileIndex
        )
            throw new ValidationError("Es ist keine Datei mit diesen Index vorhanden");
        ...
        return {
            ...
            filename: databaseBugReport.fileNames[fileIndex - 1],
    ...
    async createBugReport(
        ...
        const jsonResponse: Record<string, any> = { message: "BugReport erfolgreich erstellt" };

        const databaseBugReport = await BugReport.create({
            userId: userId,
            name: name,
            description: description,
            status: BugReportStatusType.NEW
        });
            ...
            files.map(async (file) => {
                if (file.mimetype.startsWith("image/")) {
                    ...
                    await this.s3Service.uploadFile(
                        "bug-reports",
                        `${databaseBugReport.id}/${path.parse(file.originalname).name}.webp`,
                        webpImageBuffer,
                        "image/webp"
                    );
                    ...
                    await this.s3Service.uploadFile(
                        "bug-reports",
                        `${databaseBugReport.id}/${file.originalname}`,
                        file.buffer,
                        file.mimetype
                    );
        ...
        this.socketService.emitToRoom(
            "listen:protected:bugReports:watchList",
            "bugReports:create",
            this.generateSingleJSONResponseWithModel(databaseBugReport)
        );
        this.socketService.emitToRoom(
            `listen:user:${userId}`,
            "bugReports:create",
    ...
    async updateBugReportStatus(
        ...
        routeGroups: string[],
        id: number,
        status: BugReportStatusType
    ): Promise<ControllerResponse> {
        const jsonResponse: Record<string, any> = {
            message: "Den Status vom BugReport erfolgreich geändert"
        };
        ...
        if (databaseBugReport === null)
            throw new ValidationError("Keinen BugReport mit dieser ID gefunden");
        if (status === BugReportStatusType.NEW)
            throw new ValidationError("Status kann nicht auf NEU gesetzt werden");
        if (
            databaseBugReport.userId !== userId &&
            routeGroups.includes(BUG_REPORT_WRITE.groupName) === false
        )
            throw new ForbiddenError(
                "Du kannst nur den Status von deinen eigenen BugReports bearbeiten"
            );
        if (
            routeGroups.includes(BUG_REPORT_WRITE.groupName) === false &&
            (databaseBugReport.status !== BugReportStatusType.NEW ||
                status !== BugReportStatusType.CLOSED)
        )
            throw new ForbiddenError(
                'Du kannst deine eigenen BugReports nur auf "GESCHLOSSEN" setzen wenn der Status noch auf "NEU" ist'
            );
        ...
        this.socketService.emitToRoom(
            "listen:protected:bugReports:watchList",
            "bugReports:update",
            { id: databaseBugReport.id, status: status }
        );
        this.socketService.emitToRoom("listen:public:bugReports:watchList", "bugReports:update", {
