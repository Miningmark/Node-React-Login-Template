...
export class UserManagementService {
    ...
    async getUsers(limit?: number, offset?: number): Promise<ControllerResponse> {
        const jsonResponse: Record<string, any> = {
            message: "Alle angeforderten Benutzer zurück gegeben"
        };
    ...
    async getPermissions(): Promise<ControllerResponse> {
        const jsonResponse: Record<string, any> = { message: "Alle Permissions zurück gegeben" };
    ...
    async updateUser(
        ...
        const jsonResponse: Record<string, any> = { message: "Benutzer erfolgreich bearbeitet" };
        ...
        const databaseUserTokens = await UserToken.findAll({
            where: {
                ...
                expiresAt: { [Op.gte]: new Date(Date.now()) }
            }
        });

        if (databaseUser.username.toLowerCase() === "SuperAdmin".toLowerCase())
            throw new ForbiddenError("SuperAdmin kann nicht bearbeitet werden");

        if (username !== undefined) {
            const databaseUser = await User.findOne({ where: { username: username } });
            if (databaseUser !== null)
                throw new ValidationError("Benutzername oder bereits vergeben");
        }
        ...
        if (permissionIds !== undefined) {
            const databaseSuperAdminPermission = await Permission.findOne({
                where: { name: "SuperAdmin Berechtigung" }
            });
            if (databaseSuperAdminPermission === null)
                throw new ValidationError("SuperAdmin Berechtigung nicht gefunden!");

            if (permissionIds.includes(databaseSuperAdminPermission.id))
                throw new ForbiddenError(
                    "SuperAdmin Berechtigung kann keinem anderen Benutzern zugewiesen werden"
                );
        }

        const onlyPermissionIds =
            username === undefined && email === undefined && isActive === undefined;
        if (
            !onlyPermissionIds &&
            databaseUser.isActive === false &&
            databaseUserTokens.length !== 0
        )
            throw new ForbiddenError(
                "Es können nur Rechte editiert und der Benutzer gesperrt werden solange der Registrierungsprozess nicht abgeschlossen ist"
            );
    ...
    async createUser(
        ...
        if (permissionIds.includes(databaseSuperAdminPermission.id))
            throw new ForbiddenError(
                "SuperAdmin Berechtigung kann keinen anderen Benutzern zugewiesen werden"
            );
        ...
        const token = await this.tokenService.generateHexUserToken(
            databaseUser.id,
            UserTokenType.ADMIN_REGISTRATION_TOKEN,
            ENV.ACCOUNT_ACTIVATION_ADMIN_EXPIRY
        );
