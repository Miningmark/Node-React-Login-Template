...
export abstract class BaseController {
    protected async handleRequest(
        ...
        res: Response,
        next: NextFunction,
        serviceFunction: () => Promise<ControllerResponse>
    ): Promise<void> {
        try {
            ...
            switch (controllerResponse.type) {
                ...
                case "stream":
                    ApiResponse.sendStreamSuccess(
                        res,
                        req,
                        controllerResponse.contentType,
                        controllerResponse.stream,
                        controllerResponse.filename,
                        controllerResponse.jsonResponse
                    );
                    break;
