  ...
  try {
    ...
    if (!foundUser.isActive)
      throw new ForbiddenError(
        "Dieser Benutzer ist noch nicht aktiviert oder wurde vorübergehend deaktiviert"
      );
...
const requestPasswordReset = async (req, res, next, sendResponse = true) => {
  try {
    ...
    if (!foundUser)
      throw new ValidationError("Es existiert kein Benutzer mit dieser Nutzernamen oder Email");
    if (foundUser.isDisabled)
      throw new UnauthorizedError(
        "Benutzer ist gesperrt, kein zurücksetzten des Passworts möglich"
      );
    ...
    if (userToken.expiresAt !== null) {
      if (new Date(Date.now()) > userToken.expiresAt) {
        ...
        throw new ValidationError(
          "Token bereits abgelaufen, neuer Token wurde an deine Email gesendet"
        );
      }
    }
    const hashedPassword = await bcrypt.hash(password, 10);
...
const refreshAccessToken = async (req, res, next) => {
  try {
    ...
    const accessUserToken = await findUserToken(
...
const changePassword = async (req, res, next) => {
  try {
    ...
    if (!username || !currentPassword || !newPassword)
      throw new ValidationError("Alle Eingaben erforderlich");
    ...
    if (!PASSWORD_REGEX.test(newPassword))
      throw new ValidationError("Neues Passwort entspricht nicht den Anforderungen");
...
const changeEmail = async (req, res, next) => {
  try {
    ...
    if (foundUser.email.toLowerCase() === newEmail.toLowerCase())
      throw new ConflictError("Die Email ist die selbe wie momentan verwendet");
    if (!EMAIL_REGEX.test(newEmail))
      throw new ValidationError("Neue Email entspricht keiner gültigen Email");

    const foundUserNewEmail = await findUser(null, newEmail);
    if (foundUserNewEmail && foundUserNewEmail.username !== foundUser.username)
      throw new ConflictError("Email bereits vergeben");
...
const changeUsername = async (req, res, next) => {
  try {
    ...
    if (!USERNAME_REGEX.test(newUsername))
      throw new ValidationError("Neuer Nutzername ist nicht gültig");

    const foundUserNewUsername = await findUser(newUsername, null);
    if (foundUserNewUsername && foundUserNewUsername.email !== foundUser.email)
...
const addLastLogin = async (req, userId, successfully) => {
  ...
  jsonResult.country =
    ipLookupData.status === "success" ? ipLookupData.country : "IP Lookup nicht erfolgreich";
  jsonResult.regionName =
    ipLookupData.status === "success" ? ipLookupData.regionName : "IP Lookup nicht erfolgreich";
